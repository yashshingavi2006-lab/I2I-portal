...
-- 16_phase3_review_and_notify.sql.
--
...
-- relied on right now (nothing currently drains it on a schedule — see
-- README). This table is what the bell in every portal actually reads.
--
-- Hooks into the SAME trigger functions that already enqueue email/WhatsApp
-- notifications (create or replace, same pattern already used by
-- 10_scale_fixes.sql / 11_revert_project_code_format.sql to update earlier
-- trigger logic in place) so there's one source of truth for "what happened"
-- — this migration does not touch the email/WhatsApp side of those
...
-- leader (who their mentor/ambassador is)
-- ----------------------------------------------------------------------------
